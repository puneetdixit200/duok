...
async function checkLessonAnswer(user: ReturnType<typeof userEvent.setup>, goNext = true) {
...
async function answerHelloAndThanksTranslateOne(user: ReturnType<typeof userEvent.setup>, goNext = true) {
  expect(screen.getByText('Translate')).toBeInTheDocument()
  await user.click(screen.getByRole('button', { name: 'Hello sir' }))
  await checkLessonAnswer(user, goNext)
}

async function answerHelloAndThanksTranslateTwo(user: ReturnType<typeof userEvent.setup>, goNext = true) {
  expect(screen.getByText('Translate')).toBeInTheDocument()
  await user.click(screen.getByRole('button', { name: 'Thank you' }))
  await checkLessonAnswer(user, goNext)
}

async function answerHelloAndThanksListening(user: ReturnType<typeof userEvent.setup>, goNext = true) {
  expect(screen.getByText('Listening')).toBeInTheDocument()
  const listeningChoices = screen.getByLabelText(/Listening choices/i)
  await user.click(within(listeningChoices).getByRole('button', { name: /namaskara saar/i }))
  await checkLessonAnswer(user, goNext)
}

async function answerHelloAndThanksMatchPairs(user: ReturnType<typeof userEvent.setup>, goNext = true) {
  expect(screen.getByText('Match pairs')).toBeInTheDocument()
  const matchGrid = screen.getByLabelText(/Match pairs/i)
  await user.click(within(matchGrid).getByRole('button', { name: /ನಮಸ್ಕಾರ/i }))
  await user.click(within(matchGrid).getByRole('button', { name: /^Hello$/i }))
  await user.click(within(matchGrid).getByRole('button', { name: /ಧನ್ಯವಾದ/i }))
  await user.click(within(matchGrid).getByRole('button', { name: /^Thank you$/i }))
  await user.click(within(matchGrid).getByRole('button', { name: /ಹೋಗಿ ಬನ್ನಿ/i }))
  await user.click(within(matchGrid).getByRole('button', { name: /^Goodbye$/i }))
  await user.click(within(matchGrid).getByRole('button', { name: /ಬಾ/i }))
  await user.click(within(matchGrid).getByRole('button', { name: /^Come$/i }))
  if (goNext) {
    await user.click(screen.getByRole('button', { name: /next exercise/i }))
  }
}

async function answerHelloAndThanksSpeaking(user: ReturnType<typeof userEvent.setup>, goNext = true) {
...
async function answerHelloAndThanksArrange(user: ReturnType<typeof userEvent.setup>, goNext = false) {
  expect(screen.getByText('Arrange words')).toBeInTheDocument()
  ...
  await user.click(within(wordBank).getByRole('button', { name: /ಧನ್ಯವಾದ/i }))
  await checkLessonAnswer(user, goNext)
}

async function advanceToHelloAndThanksListening(user: ReturnType<typeof userEvent.setup>) {
  await answerHelloAndThanksTranslateOne(user)
  await answerHelloAndThanksTranslateTwo(user)
}

async function advanceToHelloAndThanksMatchPairs(user: ReturnType<typeof userEvent.setup>) {
  await advanceToHelloAndThanksListening(user)
  await answerHelloAndThanksListening(user)
}

async function advanceToHelloAndThanksSpeaking(user: ReturnType<typeof userEvent.setup>) {
  await advanceToHelloAndThanksMatchPairs(user)
  await answerHelloAndThanksMatchPairs(user)
}

async function advanceToHelloAndThanksArrange(user: ReturnType<typeof userEvent.setup>) {
  await advanceToHelloAndThanksSpeaking(user)
  await answerHelloAndThanksSpeaking(user)
}
...
describe('KannadaOS desktop app', () => {
  ...
  it('shows grammar tips before starting the first untouched lesson in a unit', async () => {
    ...
    await completeOnboarding(user)
    await user.click(screen.getByRole('button', { name: /learn/i }))
    await user.click(screen.getByRole('button', { name: /Learn unit 1 lesson 1: Hello & Thanks, 0 crowns/i }))
  ...
  it('shows English subtitles for Unit 4 food-ordering tips', async () => {
    const user = userEvent.setup()
    localStorage.setItem('kannadaos:onboarded', 'true')
    render(<App />)
    ...
    const foodUnit = screen.getByText(/Unit 4: Food & Ordering/i).closest('article')
    expect(foodUnit).not.toBeNull()
    await user.click(within(foodUnit!).getByRole('button', { name: /^Tips$/i }))

    const tipsDialog = screen.getByRole('dialog', { name: /Tips: Food & Ordering/i })
    expect(within(tipsDialog).getByText(/^English: give me$/i)).toBeInTheDocument()
    expect(within(tipsDialog).getByText(/^Say: kodi$/i)).toBeInTheDocument()
    expect(within(tipsDialog).getByText(/^English: Please give one dosa$/i)).toBeInTheDocument()
    expect(within(tipsDialog).getByText(/^English: tea$/i)).toBeInTheDocument()
  ...
  it('shows English subtitles for Unit 6 PG-life tips', async () => {
    const user = userEvent.setup()
    localStorage.setItem('kannadaos:onboarded', 'true')
    render(<App />)
    ...
    const homeUnit = screen.getByText(/Unit 6: Home & PG Life/i).closest('article')
    expect(homeUnit).not.toBeNull()
    await user.click(within(homeUnit!).getByRole('button', { name: /^Tips$/i }))

    const tipsDialog = screen.getByRole('dialog', { name: /Tips: Home & PG Life/i })
    expect(within(tipsDialog).getByText(/^English: There is a problem$/i)).toBeInTheDocument()
    expect(within(tipsDialog).getByText(/^English: There is a water problem$/i)).toBeInTheDocument()
    expect(within(tipsDialog).getAllByText(/^English: There is no water$/i)).toHaveLength(2)
    expect(within(tipsDialog).getByText(/^English: Water is not coming$/i)).toBeInTheDocument()
  ...
  it('adds English subtitles to Kannada correct answers after a wrong lesson answer', async () => {
    ...
    const feedbackPanel = screen.getByRole('status', { name: /Wrong answer feedback/i })
    expect(within(feedbackPanel).getByText('Correct answer: English: Thank you | Say: dhanyavada | Kannada: ಧನ್ಯವಾದ')).toBeInTheDocument()
  })
  ...
  it('plays bundled sound effects for lesson feedback and completion', async () => {
    ...
    await answerHelloAndThanksArrange(user)
  ...
  it('shows English-readable subtitles for Kannada exercise text and choices', async () => {
    ...
    await completeOnboarding(user)
    await user.click(screen.getByRole('button', { name: /Continue: Hello & Thanks/i }))
    await advanceToHelloAndThanksArrange(user)

    expect(screen.getAllByText(/namaskara saar dhanyavada/i).length).toBeGreaterThanOrEqual(1)
    const wordBank = screen.getByLabelText(/Word bank/i)
    ...
    expect(within(wordBank).getByRole('button', { name: /thank you.*ಧನ್ಯವಾದ.*dhanyavada/i })).toBeInTheDocument()
  ...
  it('shows an English-first word guide for Kannada arrange choices', async () => {
    ...
    expect(within(wordGuide).getByText(/^thank you$/i)).toBeInTheDocument()
    expect(within(wordGuide).getByText(/^want$/i)).toBeInTheDocument()
  ...
  it('keeps listening answer text hidden until the learner checks', async () => {
    ...
    const listeningChoices = screen.getByLabelText(/Listening choices/i)
    expect(within(listeningChoices).queryByText(/ನಮಸ್ಕಾರ ಸಾರ್/i)).not.toBeInTheDocument()
    await user.click(within(listeningChoices).getByRole('button', { name: /namaskara saar/i }))
    await user.click(screen.getByRole('button', { name: /check/i }))

    expect(within(listeningChoices).getByText(/ನಮಸ್ಕಾರ ಸಾರ್/i)).toBeInTheDocument()
  })
  ...
  it('allows a near-miss typeKannada answer to retry without losing a heart', async () => {
    ...
    expect(screen.getByText('Type Kannada')).toBeInTheDocument()
  ...
  it('marks script academy lessons complete for learners who can read Kannada script', async () => {
    ...
    expect(screen.getByRole('button', { name: /Learn unit 0 lesson 1: Vowels Part 1, 1 crowns/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Learn unit 0 lesson 9: Reading Practice, 1 crowns/i })).toBeInTheDocument()
  ...
  it('puts English subtitles first on Kannada exercise choices', async () => {
    ...
    expect(screen.getByRole('button', { name: /^English: Thank you\s+ಧನ್ಯವಾದ\s+Say: dhanyavada/i })).toBeInTheDocument()
  })
  ...
  it('opens one-crown lessons with reduced-option crown replay drills', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /learn/i }))
    await user.click(screen.getByRole('button', { name: /Learn unit 1 lesson 1: Hello & Thanks, 1 crowns/i }))
  ...
  it('times out crown-four replay drills after fifteen seconds', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Learn unit 1 lesson 1: Hello & Thanks, 3 crowns/i }))
  ...
  it('uses spec replay labels and hides Kannada text before checking listening answers', async () => {
    ...
    const listeningChoices = screen.getByLabelText(/Listening choices/i)
    expect(within(listeningChoices).queryByRole('button', { name: /ನಮಸ್ಕಾರ ಸಾರ್/i })).not.toBeInTheDocument()
    await user.click(within(listeningChoices).getByRole('button', { name: /namaskara saar/i }))
    await user.click(screen.getByRole('button', { name: /check/i }))
    expect(within(listeningChoices).getByRole('button', { name: /ನಮಸ್ಕಾರ ಸಾರ್/i })).toBeInTheDocument()
  })
  ...
  it('completes the spec Hello & Thanks lesson flow and shows the completion screen', async () => {
    const user = userEvent.setup()
    render(<App />)
  ...
  it('routes tutor chat replies through Ollama when the Ollama provider is selected', async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.endsWith('/api/generate')) {
        return {
          ok: true,
          json: async () => ({
            response: 'Friendly Anna: Try ಮೀಟರ್ ಹಾಕಿ. Say: miitar haaki. English: Put the meter.',
          }),
        }
      }
    ...
    localStorage.setItem('kannadaos:onboarded', 'true')
    localStorage.setItem('kannadaos:ai-provider', JSON.stringify({ activeProvider: 'ollama' }))
    ...
    expect(screen.getByText(/Friendly Anna: Try ಮೀಟರ್ ಹಾಕಿ/i)).toBeInTheDocument()
    expect(screen.getAllByText(/Ollama local tutor reply/i).length).toBeGreaterThanOrEqual(1)

    const generateCall = fetchMock.mock.calls.find(([input]) => String(input).endsWith('/api/generate'))
  ...
  it('renders practice flashcards, Bangalore scenarios, and profile stats', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /blr/i }))
    expect(screen.getByRole('heading', { name: /Bangalore Mode/i })).toBeInTheDocument()
    expect(screen.getByText(/Practice real daily-life conversations/i)).toBeInTheDocument()
    expect(screen.getByText(/Slang of the Day/i)).toBeInTheDocument()
    expect(screen.getByText('English: Please adjust a little')).toBeInTheDocument()
    expect(screen.getByText('ಸ್ವಲ್ಪ ಅಡ್ಜಸ್ಟ್ ಮಾಡಿ')).toBeInTheDocument()
    expect(screen.getByText('Say: swalpa adjust maadi')).toBeInTheDocument()
    expect(screen.getByRole('img', { name: /BMTC Bus beginner scenario icon/i })).toHaveTextContent('🚌')
  ...
  it('generates an AI practice exercise from the Practice tab', async () => {
    ...
    expect((await within(aiPracticeCard).findAllByText(/Offline: Translate this phrase:/i)).length).toBeGreaterThanOrEqual(1)
    expect(within(aiPracticeCard).getByText('English: Hello sir')).toBeInTheDocument()
    expect(within(aiPracticeCard).getByText('Say: namaskara saar')).toBeInTheDocument()
    expect(within(aiPracticeCard).getAllByText(/Skill: greetings/i).length).toBeGreaterThanOrEqual(1)
    expect(within(aiPracticeCard).getAllByText(/Vocab: namaskara-saar/i).length).toBeGreaterThanOrEqual(1)
    expect(aiPracticeCard).toHaveTextContent(/1\s*saved\s*drill\s*for curriculum review/i)
    await waitFor(() => {
      expect(localStorage.getItem('kannadaos:ai-expansion')).toContain('Translate this phrase:')
    })
  })
  ...
  it('renders the frontend spec profile settings rows', async () => {
    ...
    expect(dailyGoalSelect).toHaveValue('10')
    expect(within(dailyGoalSelect).getByRole('option', { name: /Casual.*5 min\/day - 5 XP/i })).toBeInTheDocument()
    expect(within(dailyGoalSelect).getByRole('option', { name: /Serious.*15 min\/day - 20 XP/i })).toBeInTheDocument()
    expect(within(dailyGoalSelect).getByRole('option', { name: /Intense.*20 min\/day - 30 XP/i })).toBeInTheDocument()
    await user.selectOptions(dailyGoalSelect, '20')
    expect(within(settings).getByText(/Serious - 15 min \/ day - 20 XP/i)).toBeInTheDocument()
    await waitFor(() => expect(localStorage.getItem('kannadaos:learner-profile')).toContain('"dailyGoalXp":20'))
    ...
    expect(screen.getByRole('progressbar', { name: /XP ring: 0 of 20 XP/i })).toBeInTheDocument()
  })
  ...
  it('shows English subtitles for Kannada correct answers in review feedback', async () => {
    ...
    const reviewProgress = within(reviewSession).getByLabelText(/Review progress: 2 of 2 due words/i)
    expect(within(reviewProgress).getByLabelText('Review 1 of 2: completed')).toBeInTheDocument()
    expect(within(reviewProgress).getByLabelText('Review 2 of 2: wrong')).toBeInTheDocument()
  ...
  it('restores one heart after three correct review practice answers', async () => {
    ...
    let reviewSession = screen.getByRole('region', { name: /Review Session/i })
    let reviewProgress = within(reviewSession).getByLabelText(/Review progress: 1 of 3 due words/i)
    expect(within(reviewProgress).getAllByLabelText(/Review \d of 3:/i)).toHaveLength(3)
    expect(within(reviewProgress).getByLabelText('Review 1 of 3: current')).toBeInTheDocument()
    expect(within(reviewProgress).getByLabelText('Review 2 of 3: pending')).toBeInTheDocument()
    expect(within(reviewSession).getByText(/Choose the meaning/i)).toBeInTheDocument()
    await user.click(within(reviewSession).getByRole('button', { name: /Thank you/i }))
    await user.click(within(reviewSession).getByRole('button', { name: /Check Review/i }))
    expect(within(reviewSession).getByText(/Correct/i)).toBeInTheDocument()
    reviewProgress = within(reviewSession).getByLabelText(/Review progress: 1 of 3 due words/i)
    expect(within(reviewProgress).getByLabelText('Review 1 of 3: completed')).toBeInTheDocument()
    await user.click(within(reviewSession).getByRole('button', { name: /Next Review/i }))

    reviewSession = screen.getByRole('region', { name: /Review Session/i })
    reviewProgress = within(reviewSession).getByLabelText(/Review progress: 2 of 3 due words/i)
    expect(within(reviewProgress).getByLabelText('Review 1 of 3: completed')).toBeInTheDocument()
    expect(within(reviewProgress).getByLabelText('Review 2 of 3: current')).toBeInTheDocument()
    expect(within(reviewSession).getByText(/Complete the Kannada phrase/i)).toBeInTheDocument()
    await user.click(within(reviewSession).getByRole('button', { name: /ಹೋಗಬೇಕು/i }))
    await user.click(within(reviewSession).getByRole('button', { name: /Check Review/i }))
    expect(within(reviewSession).getByText(/Correct/i)).toBeInTheDocument()
    reviewProgress = within(reviewSession).getByLabelText(/Review progress: 2 of 3 due words/i)
    expect(within(reviewProgress).getByLabelText('Review 2 of 3: completed')).toBeInTheDocument()
    await user.click(within(reviewSession).getByRole('button', { name: /Next Review/i }))

    reviewSession = screen.getByRole('region', { name: /Review Session/i })
    reviewProgress = within(reviewSession).getByLabelText(/Review progress: 3 of 3 due words/i)
    expect(within(reviewProgress).getByLabelText('Review 3 of 3: current')).toBeInTheDocument()
    ...
    expect(within(reviewSession).queryByText('ನಮಸ್ಕಾರ ಸಾರ್')).not.toBeInTheDocument()
    const reviewLivePreview = within(reviewSession).getByRole('region', { name: /Review Kannada live preview/i })
    expect(within(reviewLivePreview).getByText(/Live preview appears here/i)).toBeInTheDocument()
    await user.type(reviewTypingInput, 'namaskara saar')
    expect(within(reviewLivePreview).getByText(/ನಮಸ್ಕಾರ ಸಾರ್/i)).toBeInTheDocument()
    fireEvent.keyDown(reviewTypingInput, { key: 'Enter' })
    expect(within(reviewSession).getByText(/Correct/i)).toBeInTheDocument()
    reviewProgress = within(reviewSession).getByLabelText(/Review progress: 3 of 3 due words/i)
    expect(within(reviewProgress).getByLabelText('Review 3 of 3: completed')).toBeInTheDocument()
  ...
  it('auto-plays speaking exercise reference audio through Web Speech when Piper is unavailable', async () => {
    ...
    expect(screen.getByText('Speaking')).toBeInTheDocument()
    await waitFor(() => expect(speak.mock.calls.length).toBeGreaterThanOrEqual(4))

...
`npm run dev` starts Vite and Electron together. `npm run test:e2e` builds the renderer, launches Electron, completes onboarding, answers a lesson, checks chat fallback plus persisted conversation reload and scenario/persona voice input, opens adaptive practice, scores pronunciation, completes a story quiz, opens Bangalore Mode, opens profile achievements, daily reminders, data export, and desktop learner storage, verifies model setup controls, checks local native model and executable paths through IPC, runs native command smoke checks, generates an exercise through the native llama.cpp bridge, and probes Ollama live status.

...
async function main() {
  ...
  try {
    ...
    await page.getByRole('combobox', { name: /^Persona$/i }).selectOption('grammar-teacher')
    ...
    await page.locator('[aria-label="Pronunciation result"]').getByText(/Score: 100 \/ 100/i).waitFor()
    await page.locator('[aria-label="Pronunciation result"]').getByText(/Level: Clear/i).waitFor()
    await page.locator('[aria-label="Pronunciation result"]').getByText(/Transcript:/i).waitFor()
    await page.locator('[aria-label="Pronunciation result"]').getByText(/Problem syllables:\s*None/i).waitFor()
    const pronunciationHistory = page.locator('[aria-label="Pronunciation history"]')
    await pronunciationHistory.getByText(/Latest: 100 \(Clear\)/i).waitFor()
    await pronunciationHistory.getByText('ನಮಸ್ಕಾರ ಸಾರ್', { exact: true }).first().waitFor()
    await pronunciationHistory.getByText('English: Hello sir', { exact: true }).first().waitFor()
    await pronunciationHistory.getByText('Say: namaskara saar', { exact: true }).first().waitFor()
    ...
    await page.getByText(/Native: Native commute drill/i).first().waitFor()
    await page.getByText(/^English: need to go$/i).first().waitFor()
    await page.getByText(/^Say: hogbeku$/i).first().waitFor()
    await page.getByRole('button', { name: /practice/i }).click()
...
async function completeLesson(page) {
  ...
  await page.getByRole('button', { name: /ಹೋಗಿ ಬನ್ನಿ/ }).click()
